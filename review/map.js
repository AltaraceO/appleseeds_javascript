...
const realNum = numbers.map(function (n) {
  return Number(n);
});

console.log(realNum);

floatNumbers = ["2.5", "5.9", "100.33"];

const floatNum = floatNumbers.map(function (n) {
  return parseFloat(n);
});
...
const movieNames = movies.map(function (n) {
  return n.name;
});

console.log(movieNames);

const movieObj = movies.map(function (n) {
  return { movieName: n.name, movieGenre: n.genres };
});
