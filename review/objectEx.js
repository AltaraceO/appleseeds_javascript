...
carMarket.getCustomerByName = function (name) {
  return this.customers.find((person) => {
...
carMarket.getAllAgencies = function () {
  return (mapAgencies = this.sellers.map((el) => {
    return el;
  }));
};
...
carMarket.getAllCustomers = function () {
  return (mapAgencies = this.customers.map((el) => {
    return el.name;
  }));
};
...
carMarket.getAllCarToBuyByAgencyId = function (id) {
  const agencyCars = this.getAgencyByID(id).cars;
  const models = agencyCars.map((carsInnerObj) => {
    return carsInnerObj.models;
  });
  return models.flat();
};
...
carMarket.getAllCarsToBuy = function () {
  ...
  this.sellers.forEach((item) => {
    result.push(...this.getAllCarToBuyByAgencyId(item.agencyId));
  });
  return result;
};
